...
- stratum-security: time-based CSRF verification via `CsrfKey` — stateless HMAC-SHA256-signed tokens (OWASP HMAC-based token pattern). The issue time is signed into the token's payload, so it cannot be tampered with; `CsrfKey::verify` checks the signature in constant time before rejecting tokens older than a caller-chosen max age (`DEFAULT_MAX_AGE` = 2h). `CsrfError` distinguishes Malformed/BadSignature/Expired. Optional session binding via `issue_for(session_id)` / `verify_for(..)` signs a session/user id into the token (not stored in it) to prevent token transplant between users. HMAC is built on the crate's existing SHA-256 (no new deps) and validated against RFC 4231 vectors. The secret is a per-deployment runtime value (`generate`/`from_secret`), never committed. The original `CsrfToken` (synchronizer pattern, no expiry) is retained (#1)
