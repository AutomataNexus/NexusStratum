...
#[component]
pub fn PaginationContent(#[props(default = String::new())] class: String, children: Element) -> Element {
    let classes = format!("flex flex-row items-center gap-1 {class}");
    rsx! { ul { class: "{classes}", {children} } }
}

#[component]
pub fn PaginationItem(#[props(default = String::new())] class: String, children: Element) -> Element {
...
#[component]
pub fn PaginationPrevious(href: String, #[props(default = String::new())] class: String) -> Element {
    let classes = format!("gap-1 pl-2.5 inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors hover:bg-accent hover:text-accent-foreground h-9 px-4 py-2 {class}");
    rsx! {
...
pub fn PaginationNext(href: String, #[props(default = String::new())] class: String) -> Element {
    let classes = format!("gap-1 pr-2.5 inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors hover:bg-accent hover:text-accent-foreground h-9 px-4 py-2 {class}");
    rsx! {
