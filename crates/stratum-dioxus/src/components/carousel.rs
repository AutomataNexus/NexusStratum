...
pub fn CarouselPrevious(
    ...
    let classes = format!(
        "absolute left-2 top-1/2 -translate-y-1/2 inline-flex items-center justify-center rounded-full border bg-background h-8 w-8 shadow-sm hover:bg-accent {class}"
    );
    rsx! {
...
pub fn CarouselNext(
    ...
    let classes = format!(
        "absolute right-2 top-1/2 -translate-y-1/2 inline-flex items-center justify-center rounded-full border bg-background h-8 w-8 shadow-sm hover:bg-accent {class}"
    );
    rsx! {
